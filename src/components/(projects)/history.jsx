      ...
      {runResult ? (
        ...
        <div className="p-6 animate-slide-in-up">
          <div className="flex items-center justify-between mb-6 animate-slide-in-down">
            ...
            <span className="pill-badge pill-badge-primary">{filteredHistory?.length || 0} entries</span>
          </div>
          ...
          <div className="flex justify-center items-center gap-3 mb-6">
            <div className="relative flex-1 max-w-md">
              <Input
                type="text"
                placeholder="Search queries by title or SQL..."
                value={historySearchTerm}
                onChange={(e) => setHistorySearchTerm(e.target.value)}
                className="pl-10 bg-white/90 border-border/30 shadow-sm focus:border-primary/40 focus:ring-primary/10 rounded-xl h-10"
              />
            ...
            <Button
              ...
              className="bg-white/90 border-border/30 shadow-sm relative cursor-pointer rounded-xl hover:bg-primary/5 hover:border-primary/30 transition-all duration-200"
            >
          ...
          {historyLoading ? (
            <div className="flex flex-col gap-4 animate-slide-in-up">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className="bg-white/80 backdrop-blur-sm shadow-sm rounded-xl p-4 border border-border/20">
            ...
            <div className="flex flex-col gap-4 stagger-children">
              {filteredHistory.map((query) => {
                ...
                return (
                  <div
                    key={query.id}
                    className="animate-slide-in-right premium-card p-5 hover:cursor-pointer"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2.5 flex-1 min-w-0">
                        <span className={`pill-badge text-xs ${
                          query.status === 'success' ? 'pill-badge-success' : 'pill-badge-error'
                        }`}>
                        ...
                        <TypeIcon className="w-3.5 h-3.5 text-primary/50" />
