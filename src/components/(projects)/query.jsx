...
export default function Query({ initialQuery, onQueryMounted }) {
  ...
  { suggestionsLoading ? (
    <div className="mx-10 mt-[25px]" aria-busy="true" aria-live="polite">
      <div className="chat-input-area bg-white/80 backdrop-blur-sm rounded-xl shadow-lg px-8 py-8 flex flex-col gap-4 border border-border/20">
        ...
        <div className="flex flex-wrap gap-3">
          {[1,2,3,4].map(n => (
            <div key={n} className="skeleton rounded-md h-10 w-1/3 max-w-[240px]" />
          ))}
        </div>
      </div>
    </div>
  ) : querysuggestions? <div className="flex flex-col h-full mt-[20px] [@media(max-width:480px)]:mt-0 animate-slide-in-up">
                      <div className={`chat-input-area premium-card mx-10 px-8 py-8 flex flex-col gap-6 [@media(max-width:480px)]:mx-2 [@media(max-width:480px)]:mt-0 ${loading ? 'loading' : ''}`}>
                        <div className="query_head flex flex-col gap-3">
                        <div className="flex items-center gap-3 mb-1">
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{background: 'linear-gradient(135deg, #133E87 0%, #608BC1 100%)'}}>
                            <Sparkles className="w-4 h-4 text-white"/>
                          </div>
                          <p className="text-gradient font-bold text-xl">Ask Your Database</p>
                        </div>
                            <textarea
                            value={query} 
                            onChange={e => setQuery(e.target.value)}
                            placeholder="e.g. Show all employees in HR department hired after 2020..."
                            className="premium-textarea min-h-[56px] max-h-[200px] overflow-auto text-gray-800"
                            rows={2}
                          />
                        ...
                        <div className="flex flex-wrap gap-2 mb-4 [@media(max-width:1400px)]:w-full stagger-children">
